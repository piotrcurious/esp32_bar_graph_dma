...
#define ADC_SAMPLE_FREQ_HZ    43036UL   // requested ADC sample frequency
...
static volatile float plot_rate_hz = 12.50f;
//static volatile float plot_rate_hz = 24.3f;
...
void adc_reader_task(void *pv)
{
  ...
  while (1) {
    ...
    if ((now - lastPlotTick) >= period_ticks) {
      ...
      // Attempt to hand snapshot to plotter. Wait only for a short bounded time
      // so we don't block the generation schedule.
      const uint32_t max_wait_ms = min<uint32_t>( (uint32_t)( (1000.0f / plot_rate_hz) / 2 ), 10 ); // at most half period or 10ms
      uint32_t waited = 0;
     // while ( (plotter.isPlotting() || !plotter.isQueueEmpty()) && (waited < max_wait_ms) ) {
     //   vTaskDelay(pdMS_TO_TICKS(1));
     //   waited++;
     // }

      //if (plotter.isPlotting() || !plotter.isQueueEmpty()) {
      //  // plotter still busy after small wait -> skip this frame (drop)
      //  dropped_frames++;
      //} else {
        // start plotting (the plotter will DMA from the out buffer)
        plotter.startBarPlot(out, BAR_COUNT);
        generated_frames++;
        // do NOT wait here; letting the loop continue preserves scheduling of the generator
      //}
    }
...
void serial_plotter_helper() {
  ...
  if (input >= '1' && input <= '9') {
    ...
    switch (selected) {
      ...
      case 9: { float r = plot_rate_hz; r = max(1.0f, r - 1.0f); setPlotRateHz(r); break; }
    }
    plotter.printConfig();
  } else if (input == '=' || input == '+') {
    // increment selected
    switch (selected) {
      ...
      case 9: { float r = plot_rate_hz; r = min(120.0f, r + 1.0f); setPlotRateHz(r); break; }
    }
    plotter.printConfig();
  }
}
