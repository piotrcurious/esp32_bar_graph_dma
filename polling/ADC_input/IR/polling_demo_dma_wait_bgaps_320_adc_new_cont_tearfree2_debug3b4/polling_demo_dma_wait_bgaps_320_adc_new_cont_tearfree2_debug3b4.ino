...
// Attempt to push snapshot to plotter. If fails and pending slot empty, save as pending snapshot.
static void push_or_save_pending(uint16_t *snapshot) {
  if (!plotter.startBarPlot(snapshot, BAR_COUNT)) {
    dropped_frames++;
    // Save into pending slot if empty
        ...
        if (trigger_enabled) {
          ...
          if (crossed) {
            ...
            if ((nowms - last_immediate_copy_ms) >= minimum_immediate_copy_interval_ms) {
              // produce snapshot & attempt to push (or save pending)
              produce_snapshot_from_ring(ring, ringSize, ring_pos, out);
              push_or_save_pending(out);
