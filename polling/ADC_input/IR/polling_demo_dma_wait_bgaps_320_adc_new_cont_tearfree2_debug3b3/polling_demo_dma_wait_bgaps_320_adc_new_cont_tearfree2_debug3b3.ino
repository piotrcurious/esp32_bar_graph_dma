...
// Fallback mode: if no crossing seen for > 2*(1/fallback_copy_freq_hz) seconds,
// produce snapshots at fallback_copy_freq_hz until crossings resume.
      ...
      if ((nowms - last_trigger_time_ms) > threshold_ms) {
        ...
        if ((nowms - last_fallback_copy_ms_local) >= fb_interval) {
          ...
          if ((nowms - last_immediate_copy_ms) >= minimum_immediate_copy_interval_ms) {
            ...
            // update last_fallback_copy_ms_local to avoid immediate retriggering on next loop
            last_fallback_copy_ms_local = nowms;
          }
        }
      }
    ...
    switch (action) {
      case 0xe: // toggle trigger enabled
        ...
        if (!trigger_enabled) {
          ...
          // initialize last times so fallback doesn't immediately fire
      ...
      case 0x7: // print status
        Serial.printf("STATUS: trigger=%s slope=%d value=%u hold_ms=%u triggered_copy=%.2f fallback_copy=%.2f fallback_thresh_ms=%u\n",
                      trigger_enabled ? "ON" : "OFF", trigger_slope, trigger_value, (unsigned)trigger_hold_ms, triggered_copy_freq_hz, fallback_copy_freq_hz, (unsigned)fallback_unreliable_threshold_ms());
        break;
      default:
...
void loop() {
  ...
  if (millis() - last_print >= 1000) {
    last_print = millis();
    Serial.printf("Plot (TFT) rate: %.2f Hz | generated: %u | dropped: %u | trigger=%s | val=%u | fb_thresh_ms=%u\n",
                  plot_rate_hz, (unsigned)generated_frames, (unsigned)dropped_frames,
                  trigger_enabled ? "ON" : "OFF", trigger_value, (unsigned)fallback_unreliable_threshold_ms());
  }
  delay(1);
}
