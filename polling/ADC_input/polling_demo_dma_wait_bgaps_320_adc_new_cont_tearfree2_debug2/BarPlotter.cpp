...
static const uint8_t DEFAULT_PORCH_STEP = 2;   // step size when nudging porches
    ...
    while (1) {
        ...
        if ((int64_t)now < wake_at) {
            ...
            if (delay_us > 2000) {
                ...
                while ((int64_t)esp_timer_get_time() < wake_at) { vTaskDelay(1); }
            }
        }

        // ensure previous DMA finished before sending start-of-frame timing
        while (self->_tft.dmaBusy()) vTaskDelay(1);
