...
#define GRID_COLOR COLOR_EXTRA  0x5502 // kolor używany dla wzoru (kratki). 
// ---------- GLOBALS ----------
...
#define GRID_COLOR COLOR_EXTRA  // kolor używany dla wzoru (kratki). Możesz zmienić.

// Modified: build each copy so that base is two-tone (top COLOR_FG, bottom COLOR_FILL)
// and then overlay the small tiled pattern using GRID_COLOR where pattern==1.
bool createTallBarSourceCopiesWithPattern(size_t copies) {
    if (copies == 0) return false;
    const size_t w = (size_t)BAR_PIXEL_WIDTH;
    const size_t h = (size_t)BAR_MAX_HEIGHT_PIXELS;
    const size_t rowsPerCopy = 2 * h; // as before
    rowWidth = w;
    ...
    Serial.printf("Precompute (tall copies with pattern): allocating %u bytes for %u pixels (w=%u, rowsPerCopy=%u, copies=%u)\n",
    ...
    // Fill each copy by two-tone base then overlay pattern (GRID_COLOR)
    for (size_t copy = 0; copy < copies; ++copy) {
        // compute vertical shift for pattern tiling (in pattern rows)
        size_t shiftRows = (copy * patternH) / copies; // możesz zmienić tę strategię
        uint16_t* basePtr = barSourceCopies + (copy * barSourcePixelsSingle);

        for (size_t r = 0; r < rowsPerCopy; ++r) {
            // base: top H rows = COLOR_FG, bottom H rows = COLOR_FILL
            uint16_t baseColor = (r < h) ? COLOR_FG : COLOR_FILL;
            size_t base = r * rowWidth;

            // determine which pattern row corresponds (with shift)
            size_t patternRow = (r + shiftRows) % patternH;

            for (size_t c = 0; c < rowWidth; ++c) {
                size_t patternCol = c % patternW;
                // start with base two-tone color
                uint16_t color = baseColor;
                // overlay pattern: where pattern==1, draw GRID_COLOR
...
void plotTask(void* pvParameters) {
    ...
    while (1) {
        ...
        // Determine which copy to use for this bar (periodicity)
        size_t copyIndex = 0;
        if (barSourceCopiesCount > 0) {
